...
namespace AlMuhasib.Infrastructure.Repositories.RealEstate;

public class RealEstateRepository<T> : IRepository<T> where T : BaseEntity
{
    private readonly IDbContextFactory<RealEstateDbContext> _contextFactory;
    private readonly Func<RealEstateDbContext?> _getActiveContext;

    public RealEstateRepository(IDbContextFactory<RealEstateDbContext> contextFactory, Func<RealEstateDbContext?> getActiveContext)
    {
        ...
        Func<IQueryable<T>, IOrderedQueryable<T>>? orderBy = null)
    {
        ...
        try
        {
            ...
            query = orderBy is not null ? orderBy(query) : query.OrderByDescending(e => e.Id);
    ...
    public IQueryable<T> Query()
    {
        ...
        return _contextFactory.CreateDbContext().Set<T>().AsQueryable();
    }
}
