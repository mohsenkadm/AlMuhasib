...
using AlMuhasib.Infrastructure.Repositories.CarTrade;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Storage;

namespace AlMuhasib.Infrastructure.Repositories;

public sealed class CarTradeUnitOfWork : IUnitOfWork
{
    private readonly IDbContextFactory<CarTradeDbContext> _contextFactory;
    private CarTradeDbContext? _activeContext;
    private IDbContextTransaction? _transaction;

    public CarTradeUnitOfWork(IDbContextFactory<CarTradeDbContext> contextFactory)
    {
        _contextFactory = contextFactory;
    }

    private CarTradeDbContext? GetActiveContext() => _activeContext;
    ...
    public IRepository<User> Users => _users ??= new CarTradeRepository<User>(_contextFactory, GetActiveContext);
    public IRepository<Permission> Permissions => _permissions ??= new CarTradeRepository<Permission>(_contextFactory, GetActiveContext);
    public IRepository<AuditLog> AuditLogs => _auditLogs ??= new CarTradeRepository<AuditLog>(_contextFactory, GetActiveContext);
    public IRepository<PrintBrandingSettings> PrintBrandingSettings => _printBrandingSettings ??= new CarTradeRepository<PrintBrandingSettings>(_contextFactory, GetActiveContext);
