...
namespace AlMuhasib.Infrastructure.Repositories.Car;

public class CarRepository<T> : IRepository<T> where T : BaseEntity
{
    private readonly IDbContextFactory<CarDbContext> _contextFactory;
    private readonly Func<CarDbContext?> _getActiveContext;

    public CarRepository(IDbContextFactory<CarDbContext> contextFactory, Func<CarDbContext?> getActiveContext)
    {
