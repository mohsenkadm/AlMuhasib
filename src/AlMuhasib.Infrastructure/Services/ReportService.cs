...
public partial class ReportService : IReportService
{
    ...
    public async Task<List<MonthlyProfitRow>> GetMonthlyProfitAsync(DateTime? from, DateTime? to)
    {
             ...
             cursor = cursor.AddMonths(1))
        {
            ...
            var sales = await InvoiceSignedSums.SumSignedNetAsync(
    ...
    public async Task<List<ProfitInvoiceDetailRow>> GetProfitInvoiceDetailsAsync(DateTime? from, DateTime? to)
    {
        ...
        foreach (var invoice in invoices)
        {
            ...
            var revenue = InvoiceFilters.SignedNetAmount(invoice);
    ...
    public async Task<CustomerStatementResult> GetCustomerStatementAsync(int customerId, DateTime? from = null, DateTime? to = null)
    {
        ...
        var installmentPayments = new List<Installment>();
        if (planIds.Count > 0)
        {
    ...
    /// <summary>يطبق سندات قبض الدين غير المعلّمة على الفواتير الآجلة (إصلاح بيانات قديمة).</summary>
    private static async Task EnsureDebtReceiptsAppliedAsync(AppDbContext context, int customerId)
    {
    ...
    public async Task<InvestorStatementResult> GetInvestorStatementAsync(int investorId, DateTime? from = null, DateTime? to = null)
    {
        ...
        var distQ = context.ProfitDistributionDetails
            .Include(d => d.ProfitDistribution)
            .Where(d => d.InvestorId == investorId);
        if (from.HasValue) distQ = distQ.Where(d => d.ProfitDistribution.Date >= from.Value);
        if (to.HasValue) distQ = distQ.Where(d => d.ProfitDistribution.Date < EndOfDay(to));
        foreach (var dist in await distQ.OrderBy(d => d.ProfitDistribution.Date).ToListAsync())
        {
            rows.Add(new InvestorStatementRow
            {
                Date = dist.ProfitDistribution.Date,
    ...
    public async Task<IncomeExpenseResult> GetIncomeExpenseReportAsync(DateTime? from, DateTime? to)
    {
        ...
        var monthlyIncomeRaw = await InvoiceFilters.ForProfitAndSalesTotals(context.Invoices, context.InstallmentPlans)
    ...
    public async Task<BalanceSheetResult> GetBalanceSheetAsync(DateTime date)
    {
        ...
        // LIABILITIES — ذمم الموردين = متبقي المشتريات الآجلة − سندات صرف غير مطبّقة (دينار فقط)
        ...
        DateTime? from, DateTime? to, int? warehouseId, bool ascending = false, int? topN = null)
    {
        ...
        var purchaseQtyByProduct = purchaseItems
            ...
            .ToDictionary(g => g.Key, g => g.Sum(x =>
                x.Invoice!.InvoiceType == InvoiceType.PurchaseReturn ? -Math.Abs(x.Quantity) : x.Quantity));
    ...
    public async Task<CustomersOverviewReportResult> GetCustomersOverviewReportAsync(DateTime? from, DateTime? to)
    {
        ...
        foreach (var customer in customers)
        {
            ...
            var salesAmount = InvoiceFilters.SumSignedNet(invoices);
    ...
    public async Task<SuppliersOverviewReportResult> GetSuppliersOverviewReportAsync(DateTime? from, DateTime? to)
    {
        ...
        {
            var invQ = InvoiceFilters.ForPurchasesTotals(
            ...
            // صافي المدفوع نقداً عبر الفواتير = مشتريات نقدية − مرتجعات مشتريات نقدية
            paid += invoices
                .Where(i => i.PaymentMethod == PaymentMethod.Cash)
                .Sum(InvoiceFilters.SignedNetAmount);
        ...
        DateTime? from, DateTime? to, int? warehouseId, int? productId)
    {
        ...
        var grouped = productIds
            .Select(pid =>
            {
                ...
                decimal qtyIn = items
                    .Where(x => x.inv.InvoiceType == InvoiceType.Purchase)
                    .Sum(x => x.ii.Quantity)
                    - items
                    .Where(x => x.inv.InvoiceType == InvoiceType.PurchaseReturn)
                    .Sum(x => Math.Abs(x.ii.Quantity));
                decimal qtyOut = items
                    .Where(x => x.inv.InvoiceType is InvoiceType.Sale or InvoiceType.Installment or InvoiceType.Damage)
                    .Sum(x => x.ii.Quantity)
                    - items
                    .Where(x => x.inv.InvoiceType == InvoiceType.SaleReturn)
                    .Sum(x => Math.Abs(x.ii.Quantity));
        ...
        InventoryReplenishmentFilter filter = InventoryReplenishmentFilter.All)
    {
        ...
        var replenishmentProductIds = stocks.Select(s => s.ProductId)
            ...
            .Distinct()
            .ToList();
        var purchasesByProduct = await ProductCostHelper.GetPurchaseItemsByProductAsync(context, replenishmentProductIds);
    ...
    private static InventoryReplenishmentRow? BuildReplenishmentRow(
        ...
        IReadOnlyList<InvoiceItem> purchaseItems)
    {
