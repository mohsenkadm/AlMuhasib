...
public sealed class CarTradeSyncService : ISyncService, IDisposable
{
    private const string GlobalSyncKey = "CarTradeGlobal";
    ...
    private async Task<SyncRunResult> SyncNowCoreAsync(CancellationToken ct)
    {
        ...
        await CarTradeSyncIdEnsurer.EnsureAllAsync(db, ct);
        var syncState = await GetOrCreateSyncStateAsync(db, ct);

        var pushBundle = await CarTradeSyncMapper.BuildPushBundleAsync(db, syncState.LastPushedAt, ct);
        ...
        try
        {
            await CarTradeSyncMapper.ApplyPullBundleAsync(applyDb, pullResult.Data, ct);
    ...
    private static async Task<SyncState> GetOrCreateSyncStateAsync(CarTradeDbContext db, CancellationToken ct)
    {
