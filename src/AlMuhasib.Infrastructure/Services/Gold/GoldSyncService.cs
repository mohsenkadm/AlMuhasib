...
/// <summary>Minimal Gold sync stub — empty push/pull bundles until Gold sync DTOs are added.</summary>
public sealed class GoldSyncService : ISyncService, IDisposable
{
    private const string GlobalSyncKey = "GoldGlobal";
    ...
    private async Task<SyncRunResult> SyncNowCoreAsync(IProgress<SyncProgressUpdate>? progress, CancellationToken ct)
    {
        ...
        // Stub: empty push bundle until Gold sync mappers exist
        var pushBundle = new SyncDataBundle();
        ...
        try
        {
            // Stub: no Gold pull mapper yet — update sync cursors only
            _ = pullResult.Data;
    ...
    private static async Task<SyncState> GetOrCreateSyncStateAsync(GoldDbContext db, CancellationToken ct)
    {
