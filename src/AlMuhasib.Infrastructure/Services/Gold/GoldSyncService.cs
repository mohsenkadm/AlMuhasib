...
public sealed class GoldSyncService : ISyncService, IDisposable
{
    ...
    private async Task<SyncRunResult> SyncNowCoreAsync(IProgress<SyncProgressUpdate>? progress, CancellationToken ct)
    {
        ...
        await GoldSyncIdEnsurer.EnsureAllAsync(db, ct);
        var syncState = await GetOrCreateSyncStateAsync(db, ct);

        var pushBundle = await GoldSyncMapper.BuildPushBundleAsync(db, syncState.LastPushedAt, ct);
        ...
        try
        {
            await GoldSyncMapper.ApplyPullBundleAsync(applyDb, pullResult.Data, ct);
