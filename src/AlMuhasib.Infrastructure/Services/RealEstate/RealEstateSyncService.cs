...
public sealed class RealEstateSyncService : ISyncService, IDisposable
{
    private const string GlobalSyncKey = "RealEstateGlobal";
    ...
    private async Task<SyncRunResult> SyncNowCoreAsync(IProgress<SyncProgressUpdate>? progress, CancellationToken ct)
    {
        ...
        await RealEstateSyncIdEnsurer.EnsureAllAsync(db, ct);
        var syncState = await GetOrCreateSyncStateAsync(db, ct);

        var pushBundle = await RealEstateSyncMapper.BuildPushBundleAsync(db, syncState.LastPushedAt, ct);
        ...
        try
        {
            await RealEstateSyncMapper.ApplyPullBundleAsync(applyDb, pullResult.Data, ct);
    ...
    private static async Task<SyncState> GetOrCreateSyncStateAsync(RealEstateDbContext db, CancellationToken ct)
    {
