...
public sealed class HotelSyncService : ISyncService, IDisposable
{
    private const string GlobalSyncKey = "HotelGlobal";
    ...
    private async Task<SyncRunResult> SyncNowCoreAsync(CancellationToken ct)
    {
        ...
        await HotelSyncIdEnsurer.EnsureAllAsync(db, ct);
        var syncState = await GetOrCreateSyncStateAsync(db, ct);

        var pushBundle = await HotelSyncMapper.BuildPushBundleAsync(db, syncState.LastPushedAt, ct);
        ...
        try
        {
            await HotelSyncMapper.ApplyPullBundleAsync(applyDb, pullResult.Data, ct);
    ...
    private static async Task<SyncState> GetOrCreateSyncStateAsync(HotelDbContext db, CancellationToken ct)
    {
