...
public static class DocumentPrintHelper
{
    ...
    internal sealed class PrintPreviewWindow : Window
    {
        ...
        private void OnPrintClick(object sender, RoutedEventArgs e)
        {
            ...
            // Prefer printable width so sides are not clipped, but keep A4 height from preview
            // so content that fit one page in preview does not reflow onto a second page.
            var printableWidth = printDialog.PrintableAreaWidth > 0
                ? printDialog.PrintableAreaWidth
                : DefaultPageWidth;
            var pageHeight = _document.PageHeight > 0 ? _document.PageHeight : DefaultPageHeight;
            var printSize = new Size(printableWidth, pageHeight);
            ApplyPageLayout(_document, printSize);

            var paginator = ((IDocumentPaginatorSource)_document).DocumentPaginator;
            paginator.PageSize = printSize;
