...
public partial class GoldSupplierPaymentViewModel : ViewModelBase
{
    ...
    public GoldSupplierPaymentViewModel(
        ...
        ICurrentUserService currentUserService)
    {
        ...
        PageTitle = "تسديد الموردين";
    }

    public override async Task InitializeAsync()
    {
        LoadPermissions(_currentUserService, GoldShopPermissionRegistry.SupplierPayment);
        await LoadAsync();
    }
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            var (open, _) = await _purchaseService.GetPagedAsync(1, 200, SearchText, status: GoldInvoiceStatus.Open);
            var (partial, _) = await _purchaseService.GetPagedAsync(1, 200, SearchText, status: GoldInvoiceStatus.PartiallyPaid);
    ...
    private async Task RecordPaymentAsync()
    {
        ...
        if (!CanAdd && !CanEdit)
        {
            ErrorMessage = "ليس لديك صلاحية تسديد الموردين";
            return;
        }
        ...
        try
        {
            ...
            var invoice = await _purchaseService.RecordPaymentAsync(request);
    ...
    private async Task ExportToExcel()
    {
        try
        {
            ...
            var dialog = new SaveFileDialog
            {
                ...
                FileName = $"تسديد_الموردين_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx",
                DefaultExt = ".xlsx"
            };

            if (dialog.ShowDialog() == true)
            {
                await _exportService.ExportToExcelFileAsync(exportData, dialog.FileName, "تسديد الموردين");
    ...
    private async Task PrintTable()
    {
        try
        {
            ...
            var columns = new[] { "رقم الفاتورة", "التاريخ", "المورد", "المتبقي", "العملة" };
            ...
            _exportService.PrintTable("الفواتير المفتوحة لتسديد الموردين", columns, rows);
        }
        catch (Exception ex)
        {
