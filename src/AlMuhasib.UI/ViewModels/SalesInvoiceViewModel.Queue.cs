...
public partial class SalesInvoiceViewModel
{
    ...
    private void AddToQueue()
    {
        ...
        var queueName = string.IsNullOrWhiteSpace(CustomerSearchText)
            ? $"مبيعات - {DateTime.Now:HH:mm}"
            : $"مبيعات {CustomerSearchText.Trim()} - {DateTime.Now:HH:mm}";

        _queueService.Enqueue(
            InvoiceQueueKind.Sales,
            queueName,
            BuildDraft(),
    ...
    private void LoadFromQueue(InvoiceQueueItem? item)
    {
        ...
        var draft = _queueService.Load<SalesInvoiceDraft>(item.Id);
        if (draft is null)
        {
    ...
    {
        QueueItems.Clear();
        IEnumerable<InvoiceQueueItem> items = _queueService.GetItems(InvoiceQueueKind.Sales);
