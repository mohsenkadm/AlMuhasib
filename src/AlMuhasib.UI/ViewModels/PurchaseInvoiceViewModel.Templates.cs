...
public partial class PurchaseInvoiceViewModel
{
    ...
    private void ConfirmSaveTemplate()
    {
        ...
        _templateService.SaveTemplate(BuildTemplateFromCurrent(name));
        RefreshSavedTemplates();
        IsSaveTemplateDialogOpen = false;
        NewTemplateName = string.Empty;
    ...
    [RelayCommand]
    private void SetDefaultSupplierFromInvoice()
    {
        if (SelectedSupplier is null)
        {
            BeautifulMessageDialog.ShowWarning("اختر مورداً أولاً");
            return;
        }

        _templateService.SetDefaultPurchaseSupplier(SelectedSupplier.Id);
        BeautifulMessageDialog.ShowSuccess($"تم تعيين «{SelectedSupplier.Name}» كمورد افتراضي");
    }

    private void RefreshSavedTemplates()
    {
        SavedTemplates.Clear();
        foreach (var t in _templateService.GetTemplates(InvoiceTemplateKind.Purchase))
    ...
    {
        Name = name,
        Kind = InvoiceTemplateKind.Purchase,
        SupplierId = SelectedSupplier?.Id,
        WarehouseId = SelectedWarehouse?.Id,
        PaymentMethod = IsCashPayment ? Core.Enums.PaymentMethod.Cash : Core.Enums.PaymentMethod.Credit,
    ...
    {
        if (template.SupplierId.HasValue)
        {
            SelectedSupplier = Suppliers.FirstOrDefault(s => s.Id == template.SupplierId);
        ...
        IsCashPayment = template.PaymentMethod == Core.Enums.PaymentMethod.Cash;
    ...
    private void ApplyDefaultSupplierIfAny()
    {
        var id = _templateService.GetDefaultPurchaseSupplierId();
