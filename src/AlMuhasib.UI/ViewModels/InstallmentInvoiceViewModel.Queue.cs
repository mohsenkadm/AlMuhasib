...
public partial class InstallmentInvoiceViewModel
{
    ...
    private void AddToQueue()
    {
        ...
        var queueName = string.IsNullOrWhiteSpace(CustomerSearchText)
            ? $"أقساط - {DateTime.Now:HH:mm}"
            : $"أقساط {CustomerSearchText.Trim()} - {DateTime.Now:HH:mm}";

        _queueService.Enqueue(
            InvoiceQueueKind.Installment,
            queueName,
            BuildDraft(),
    ...
    private void LoadFromQueue(InvoiceQueueItem? item)
    {
        ...
        var draft = _queueService.Load<InstallmentInvoiceDraft>(item.Id);
        if (draft is null)
        {
    ...
    {
        QueueItems.Clear();
        IEnumerable<InvoiceQueueItem> items = _queueService.GetItems(InvoiceQueueKind.Installment);
    ...
    {
        if (InvoiceNavigationBridge.PendingOpenQueueKind != InvoiceQueueKind.Installment)
            return;
