...
public partial class OpeningSupplierBalanceViewModel : ViewModelBase
{
    ...
    private async Task LoadItemsAsync()
    {
        ...
        try
        {
            ...
            {
                var allResult = await _balanceService.GetSupplierOpeningBalancesAsync(BuildQuery(1, int.MaxValue));
            ...
            {
                var result = await _balanceService.GetSupplierOpeningBalancesAsync(BuildQuery(CurrentPage, PageSize));
    ...
    private async Task OpenAddDialog()
    {
        ...
        IsEditMode = false;
        DialogTitle = "إضافة رصيد افتتاحي لمورد";
        ResetDialogForm();
    ...
    private void OpenEditDialog(OpeningPartyBalanceListItem? item)
    {
        ...
        SupplierSearchText = item.PartyName;
        SelectedSupplier = Suppliers.FirstOrDefault(s => s.Id == item.PartyId);
    ...
    private async Task SaveBalanceAsync()
    {
        ...
        {
            DialogError = "يرجى اختيار مورد أو إدخال اسمه";
            return;
        }
    ...
    private async Task ExecuteDeleteAsync()
    {
        ...
        {
            IsBusy = true;
            await _balanceService.DeleteSupplierOpeningBalanceAsync(ItemToDelete.InvoiceId);
            IsDeleteDialogOpen = false;
            ItemToDelete = null;
    ...
    private async Task PrintTable()
    {
        try
        {
            ...
            var columns = new[] { "رقم", "المورد", "الهاتف", "المبلغ", "المتبقي", "التاريخ", "ملاحظات" };
