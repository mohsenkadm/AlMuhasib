...
public partial class InstallmentInvoiceViewModel
{
    ...
    private void OpenSaveTemplateDialog()
    {
        ...
        {
            BeautifulMessageDialog.ShowWarning("أضف بنوداً أو حدّد عدد الأقساط قبل حفظ القالب");
            return;
        }

        NewTemplateName = $"قالب أقساط {DateTime.Now:yyyy/MM/dd HH:mm}";
    ...
    private void DeleteTemplate(InvoiceTemplate? template)
    {
        ...
        {
            BeautifulMessageDialog.ShowWarning("لا يمكن حذف القوالب الجاهزة المدمجة في النظام");
            return;
        }
    ...
    private void SetDefaultCustomerFromInvoice()
    {
        ...
        _templateService.SetDefaultInstallmentCustomer(SelectedCustomer.Id);
        BeautifulMessageDialog.ShowSuccess($"تم تعيين «{SelectedCustomer.Name}» كعميل افتراضي للأقساط");
    }

    private void RefreshSavedTemplates()
    {
        SavedTemplates.Clear();
        foreach (var t in _templateService.GetTemplates(InvoiceTemplateKind.Installment))
            SavedTemplates.Add(t);
    }

    private InvoiceTemplate BuildTemplateFromCurrent(string name)
    {
        var startOffset = Math.Max(0,
            ((InstallmentStartDate.Year - DateTime.Today.Year) * 12)
            + InstallmentStartDate.Month - DateTime.Today.Month);
        ...
        return new InvoiceTemplate
        {
            ...
            InstallmentStartMonthsOffset = startOffset > 0 ? startOffset : 1,
    ...
    private void ApplyTemplate(InvoiceTemplate template)
    {
        ...
        FileNumber = template.FileNumber ?? string.Empty;

        if (template.InstallmentType.HasValue)
            SelectedInstallmentType = template.InstallmentType.Value;

        if (template.NumberOfInstallments is > 0)
            NumberOfInstallments = template.NumberOfInstallments.Value;

        var offset = template.InstallmentStartMonthsOffset > 0 ? template.InstallmentStartMonthsOffset : 1;
        InstallmentStartDate = DateTime.Today.AddMonths(offset);
    ...
    private void ApplyDefaultInstallmentCustomerIfAny()
    {
        var id = _templateService.GetDefaultInstallmentCustomerId();
